...
CreateThread(function()
	local food = { "chocice", "vanice", "caramelice", "milk", "sugar", "chocchips", "vanillaescence", "mint", "strawberryice", "blueberryice", "cream", "orange", "strawberry", "blueberry", "orangeice", "mintice" }
    for k,v in pairs(food) do QBCore.Functions.CreateUseableItem(v, function(source, item) TriggerClientEvent('amari-icecreamshop:client:Eat', source, item.name) end) end
end)

RegisterServerEvent('amari-icecreamshop:GetFood', function(data)
	local src = source
...
QBCore.Functions.CreateCallback('amari-icecreamshop:get', function(source, cb, item, tablenumber, craftable)
	local src = source
	local hasitem = nil
